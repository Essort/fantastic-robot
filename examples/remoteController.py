...
def sendSocketComm(data):
    ...
    HOST, PORT = "192.168.88.252", 27015
...
while done == False:
    ...
    for i in range(joystick_count):
        ...
        if (str(joystick.get_button(2)) == "1"):
            if headY < 170:
                headY += servoStep
        if (str(joystick.get_button(4)) == "1"):
            if headY > -240:
                headY -= servoStep
        ...
        if (prevCommand != data):
            prevCommand = data
            # TODO reenable socket communication
            sendSocketComm(data)
            print(data)
